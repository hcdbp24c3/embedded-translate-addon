...
export default function Configure() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-4">
      ...
      <p className="text-gray-600">Extract built-in SRT/ASS and translate via OpenAI-compatible API. Perfect sync — tự động bắt stream.</p>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500">Hỗ trợ OpenAI, Gemini, Groq, Ollama http://localhost:11434/v1, Qwen http://66.36.226.70:8008/v1</p>
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-gray-500">Gợi ý: gpt-4o-mini, gemini-2.0-flash, Qwen3-5B, llama3.1</p>
      </div>

      <div className="bg-gray-50 p-3 rounded text-xs text-gray-600">
        <p>✓ Addon sẽ <b>tự động bắt stream URL</b> bạn đang xem (không cần nhập thêm key). Chỉ cần chọn phim, addon sẽ tự extract sub gốc và dịch.</p>
      </div>
      ...
      <div className="text-xs text-gray-500 pt-4 border-t">
        <p>Tip: URL manifest của bạn là <code>manifest.json?config=...</code>. Mở <code>/configure?config=...</code> sẽ tự động điền lại.</p>
      </div>
    </div>
  )
}
